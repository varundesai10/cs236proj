...
class AttackDataset(Dataset):
    ...
    def __len__(self):
        ...
        if not hasattr(self, 'main_indices'):
            self.main_indexes = np.arange(
                get_hd5_length(self.data_path, self.data_key))
    ...
    def _open_h5_file(self):
        ...
        self.data = self.file_archive[self.data_key]
        self.labels = self.file_archive[self.labels_key]
        if self.use_masks:
            self.masks = self.file_archive[self.masks_key]
    ...
    def __getitem__(self, index):
        ...
        x = self.data[index]
        x = x / 255. if self.rescale else x
    ...
    def get_length_and_shape(self):
        ...
        with h5py.File(self.data_path, 'r') as f:
            n = len(f[self.data_key])
            sample_shape = f[self.data_key][0].shape
